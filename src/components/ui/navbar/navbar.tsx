...
const Tab = ({
  ...
  return (
    <span className={`relative flex items-center justify-center hover:bg-[#0606069E] hover:bg-black ${hasDropdown ? 'rounded-t-2xl' : 'rounded-2xl'}`}>
    {hasDropdown ? (
      <Dropdown trigger={item.name} items={list} />
    ) : (
      <Link
        key={item.name}
        className="text-[#CDCDCD] font-semibold text-lg uppercase hover:cursor-pointer py-4 px-3 text-nowrap"
        href={href}
      >
        {item.name}
      </Link>
