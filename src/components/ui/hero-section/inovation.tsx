...
const Inovation = () => {
    return (
        <Section  className='bg-transparent'>
            ...
            <Section className='grid grid-cols-3 w-full bg-transparent  lg:m-auto content-center  gap-2 lg:gap-y-11 lg:gap-x-5 lg:w-[84rem]'>
                {
                    Innovation.map((item, index) =>
                        <div className='lg:aspect-[4/7]  w-full h-52    lg:w-full lg:h-[700px]   relative mx-auto' key={item.title}>
                            <Image src={item.image} alt='' className='object-cover ' fill />
                            <div className='absolute bottom-0 flex w-full  justify-between items-center p-2'><h2 className=' text-xxs lg:text-3xl font-bold'> 0{index + 1}</h2> <h2 className='text-[9px] lg:text-xl uppercase font-bold text-end'>{item.title}</h2></div>
                        </div>)
                }
            </Section>
        </Section>
